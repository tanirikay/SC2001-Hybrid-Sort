...
for n in n_values:
    ...
    ratio = comparison / theoretical
    ratio_results.append(ratio)

    print("n =", n,"comparisons =", comparison,"ratio =", f"{ratio:.4f}")

plt.figure()
plt.plot(n_values,comparison_results,marker = "o",label = "Empirical Hybrid Sort")
plt.xscale("log")
plt.yscale("log")
...
plt.figure()
plt.plot(n_values, ratio_results, marker="o", label="Empirical Ratio")
plt.xscale("log")

plt.axhline(y=1, linestyle="--", label="Ratio = 1")

plt.ylim(0.8, 1.2)

plt.xlabel("Input Size n")
plt.ylabel("Comparisons / (n log2 n)")
plt.title("Ratio of Empirical Comparisons to n log2(n)")

plt.legend()
plt.grid()
plt.savefig("part_c_i_ratio.png")
plt.show()
...
#----------------------
n_value_ii = 100000
s_value = [1,2,5,10,20,50,100,200]
...
print("----------------------")
for s_ii in s_value:
    arr_copy = arr_ii.copy()  # Ensure we sort the same array for each S

    comparison_ii = run_hybrid_sort(arr_copy, s_ii)
    comparison_results_ii.append(comparison_ii)

    print("S =", s_ii, "comparison =", comparison_ii)
